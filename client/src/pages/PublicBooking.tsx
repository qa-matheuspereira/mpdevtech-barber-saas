...
import { ArrowRight, CheckCircle2, Clock, User, Scissors, AlertCircle } from "lucide-react";
...
interface EstablishmentConfig {
  id: number;
  name: string;
  operatingMode: "queue" | "scheduled" | "both";
...
export default function PublicBooking() {
  ...
  // Mock barbershop configurations
  const establishments: EstablishmentConfig[] = [
    {
      id: 1,
      name: "Estabelecimento do João",
      operatingMode: "both",
      services: [
    ...
    {
      id: 3,
      name: "Estabelecimento Premium",
      operatingMode: "queue",
      services: [
        { id: 1, name: "Corte de Cabelo", duration: 30, price: 60 },
        { id: 2, name: "Barba", duration: 20, price: 40 },
        { id: 3, name: "Corte + Barba", duration: 50, price: 90 },
      ],
    },
  ];

  const selectedEstablishment = establishments.find((b) => b.id === parseInt(establishmentId));
  const selectedService = selectedEstablishment?.services.find((s) => s.id === parseInt(serviceId));

  // Set default booking mode based on barbershop configuration
  useEffect(() => {
    if (selectedEstablishment) {
      if (selectedEstablishment.operatingMode === "queue") {
        setBookingMode("queue");
      } else if (selectedEstablishment.operatingMode === "scheduled") {
        setBookingMode("scheduled");
      } else {
        // "both" - keep current selection or default to scheduled
        setBookingMode("scheduled");
      }
    }
  ...
  const canProceedToStep2 = establishmentId && serviceId;
  ...
  const getModeLabel = (mode: string) => {
    switch (mode) {
      ...
      case "both":
        return "Fila Virtual ou Horário Marcado";
      default:
        return mode;
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600">Escolha o estabelecimento, serviço e horário que melhor se adequa a você</p>
        </div>
        ...
        {currentStep === 1 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Selecione o estabelecimento e o serviço desejado</CardDescription>
            </CardHeader>
                  ...
                  <SelectContent>
                    {establishments.map((shop) => (
                      <SelectItem key={shop.id} value={shop.id.toString()}>
                        {shop.name} • {getModeLabel(shop.operatingMode)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {selectedEstablishment && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 flex gap-2">
                  ...
                  <p className="text-sm text-blue-900">
                    <strong>{selectedEstablishment.name}</strong> trabalha com: <strong>{getModeLabel(selectedEstablishment.operatingMode)}</strong>
                  </p>
                </div>
              )}
        ...
        {/* Step 3: Time/Mode Selection */}
        {currentStep === 3 && (
          <div className="space-y-4">
            {selectedEstablishment?.operatingMode === "both" ? (
              // Show mode selection for "both"
              <Card>
                <CardHeader>
                  <CardTitle>Escolha o Tipo de Agendamento</CardTitle>
                  <CardDescription>Como você prefere ser atendido?</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
            ...
            ) : selectedEstablishment?.operatingMode === "scheduled" ? (
              // Show time slots for scheduled
              <TimeSlotSelector
                date={selectedDate!}
                slots={timeSlots}
              ...
              // Queue mode - skip to confirmation
